...
\begin{enumerate}[+1)]
	\item Uživatelské rozhraní aplikace nabídne intuitivní klávesové zkratky pro usnadnění práce -- \textsf{Ctrl +} a \textsf{Ctrl -} obstará přibližování/oddalování momentálního výběru, \textsf{Ctrl n} a \textsf{Ctrl p} vybere předchozího/následujícího sourozence vybraného prvku, apod.
...
\begin{description}
	...
	\end{itemize}
	\item [Data engine] zařizuje extrakci dat z vybraných prvků na stránce (převážně tedy text, ale v případě obrázků se jedná o URL jejich zdroje a v budoucnu může být snadno rozšířeno o další možnosti) a obsahuje menší komponentu \emph{Exporter}, na kterou deleguje export dat do zvoleného formátu.
...
Dle požadavku F1 je tedy aplikace rozdělena na pracovní plochu s uživatelem zadanou stránkou a postranní panel, skrz který probíhá veškerá ovládání. Dle F2 je pak panel rozdělen na záložky obsahující jednotlivé způsoby výběru. V horní části panelu pak nalezneme ovládací tlačítka akcí undo a redo. Zbývá už jen velké tlačítko \textsf{Preview} a \textsf{Download} spolu s výběrem formátu exportu.

Následující wireframy představují návrh uživatelského rozhraní aplikace.
\begin{figure}[h]
...
\end{figure}
\begin{figure}[h]
	\includegraphics[width=\linewidth]{images/cols_wireframe.png}
	\caption{Výběr sloupců}
	\label{fig:scraper_cols_wireframe}
\end{figure}
\begin{figure}[h]
...
\begin{figure}[h]
	...
	\begin{minipage}{.5\linewidth}
		\centering
		\includegraphics[width=.9\linewidth]{images/Scraper_text_search.png}
		\caption{Výběr na základě textové shody}
